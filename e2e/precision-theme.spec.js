...
async function mockWorkspace(page, role, { empty = false, invalidLogin = false, activation = false } = {}) {
  ...
  await page.route('**/api/v1/**', async route => {
    ...
    if (path.endsWith('/workout-history')) return json({ items: [] })
    ...
    if (path === '/api/v1/admin/totals') return json({ total_coaches: 1, total_clients: 1 })
...
for (const width of [1440, 768, 390]) {
  test(`Client theme and all navigation at ${width}px`, async ({ page }, testInfo) => {
    ...
    for (const [label, heading] of [['Body Tracker', 'Track signal. See change.'], ['Check-ins', 'Check in with yourself.'], ['Progress Photos', 'Progress, in perspective.'], ['Nutrition', 'Your meals. Your rhythm.'], ['Workout', 'Upper strength'], ['Health Summary', 'Your health context, protected.'], ['Profile', 'Your plan starts with context.']]) {
  ...
  test(`Coach theme and all navigation at ${width}px`, async ({ page }, testInfo) => {
    ...
    await expect(page.getByRole('heading', { name: 'Enroll client' })).toBeVisible()
    await checkTheme(page)
    await page.screenshot({ path: testInfo.outputPath('coach-onboarding.png'), fullPage: true })
    await page.getByRole('button', { name: 'Close client enrollment' }).click()
    expect(proof.errors).toEqual([])
    expect(proof.unexpected).toEqual([])
  })
...
test('client forms preserve request contracts; dashboard shortcuts work', async ({ page }) => {
  ...
  await expect(page.getByPlaceholder('71', { exact: true })).toHaveCount(0)
  ...
  expect(proof.requests.find(r => r.method === 'PUT' && r.path.includes('/body-entries/')).data).toEqual({ weight_kg: 71.8 })
  ...
  await page.getByLabel('Weight (kg)', { exact: true }).fill('71.8')
  await page.getByLabel('What went well?', { exact: true }).fill('Consistent training.')
  await page.locator('select[name="energy_score"]').selectOption('4')
  await page.locator('select[name="sleep_score"]').selectOption('3')
  for (const name of ['energy','sleep_quality','hunger','digestion','stress','recovery','strength','workout_performance','motivation','adherence','overall_wellbeing']) await page.locator(`select[name="${name}"]`).selectOption('7')
  await page.locator('select[name="sentiment"]').selectOption('good')
  ...
  await expect(page.getByRole('heading', { name: 'Upper strength', exact: true })).toBeVisible()
  expect(proof.errors).toEqual([])
  expect(proof.unexpected).toEqual([])
})
...
test('forgot password requests ownership proof, never email-only password changes', async ({ page }) => {
  const proof = await mockWorkspace(page, 'client')
  ...
  expect(proof.requests.some(item => item.path === '/api/v1/auth/set-password')).toBeFalsy()
})
