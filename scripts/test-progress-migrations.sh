...
if [[ -f "$repo_dir/backend/tests/sql/coach_workspace_persistence_acceptance.sql" ]]; then
  psql "${psql_args[@]}" -f "$repo_dir/backend/tests/sql/coach_workspace_persistence_acceptance.sql"
fi
