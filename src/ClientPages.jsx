...
export function NutritionPage({ nutrition, onSetAdherence, onGetRecipe }) {
  ...
  const [notice, setNotice] = useState('')
  if (!nutrition) return <section className="client-page" aria-labelledby="nutrition-title">
    <PageHeading eyebrow="CLIENT / NUTRITION" title="Nutrition plan pending." copy="Your coach has not assigned a nutrition plan yet." badge="AWAITING PLAN" />
    <article className="panel"><header><div><p className="kicker">PLAN STATUS</p><span>Coach-owned nutrition appears here when it is published.</span></div></header><div className="coach-empty"><strong>No nutrition plan assigned</strong><span>Use your next check-in to share dietary context or ask your coach for a plan.</span></div></article>
  </section>
...
export function WorkoutPage({ workout, onSave }) {
  ...
  if (!workout) return <section className="client-page" aria-labelledby="workout-title">
    <PageHeading eyebrow="CLIENT / WORKOUT" title="Workout plan pending." copy="Your coach has not assigned a session for today." badge="AWAITING PLAN" />
    <article className="panel"><header><div><p className="kicker">SESSION STATUS</p><span>Coach-owned training appears here when it is scheduled.</span></div></header><div className="coach-empty"><strong>No workout assigned</strong><span>Use your next check-in to tell your coach what support or equipment you need.</span></div></article>
  </section>
