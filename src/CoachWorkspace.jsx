...
const initialClients = [
  { id: 'XP-0017', name: 'Maya Shah', initials: 'MS', weight: '68.4 kg', lastEntry: 'Today', checkIn: 'Submitted', status: 'On track', goal: 'Body recomposition', attention: false },
  { id: 'XP-0024', name: 'Arjun Mehta', initials: 'AM', weight: '82.1 kg', lastEntry: '4 days ago', checkIn: 'Due', status: 'Needs review', goal: 'Fat loss', attention: true },
  { id: 'XP-0031', name: 'Nadia Khan', initials: 'NK', weight: '—', lastEntry: '9 days ago', checkIn: 'No start date', status: 'Missing data', goal: 'Strength', attention: true },
  { id: 'XP-0034', name: 'Kabir Iyer', initials: 'KI', weight: '74.8 kg', lastEntry: 'Yesterday', checkIn: 'Submitted', status: 'On track', goal: 'Performance', attention: false },
]
...
function CoachOverview({ clients, selectClient, navigate, onCreate }) {
  ...
  return <section className="coach-page">
    <CoachHeading eyebrow="XFORMPHYSIQUE / COACH" title={<>Control system.<em> See signal.</em></>} copy="Progress intelligence and client operations in one controlled workspace." action={<button className="coach-primary" onClick={onCreate}><CoachGlyph name="plus" />Enroll client</button>} />
    ...
    <section className="panel progress-panel"><h3>Upcoming client check-ins</h3>{clients.length ? clients.map(c => <div className="progress-actions" key={c.id}><span>{c.name} · {formatDate(c.schedule?.due_on)} · {c.schedule?.current_status || 'Schedule pending'}<small> · next {formatDate(c.schedule?.next_due_on)} · {c.schedule?.timezone || ''}</small></span><button onClick={() => selectClient(c.id)}>Review {c.name}</button></div>) : <p>No assigned clients yet.</p>}</section>
  ...
  const submit = async (event) => {
    ...
    try {
      await onCreate({
        ...
        amount_paid: data.get('amountPaid') === '' ? null : Number(data.get('amountPaid')),
...
function CoachSettings({ notice, account, profilePhoto, onUploadProfilePhoto, accessToken }) {
  ...
  return (
    <section className="coach-page">
      <CoachHeading eyebrow="COACH / SYSTEM CONFIGURATION" title="Settings" copy="Weight tracking, units and safe data operations." />
...
function CoachNoClient({ loading, onCreate }) {
  return <section className="coach-page"><section className="coach-empty"><CoachGlyph name={loading ? 'trend' : 'clients'} /><strong>{loading ? 'Loading your authorized clients…' : 'No client workspace is available yet.'}</strong><span>{loading ? 'The roster is being checked against your coach assignment.' : 'Enroll a client, then their body entries and check-ins will appear here.'}</span>{!loading && <button className="coach-primary" type="button" onClick={onCreate}>Enroll client</button>}</section></section>
}
  ...
  const saveSetup = async (event) => {
    ...
    try {
      const setup = await coachApi.updateClientSetup(client.id, {
        ...
        amount_paid: setupDraft.amount_paid === '' ? null : Number(setupDraft.amount_paid),
